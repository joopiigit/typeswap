...
static void CreateBattlerSprite(u8 battler)
{
    if (battler < gBattlersCount)
    {
        ...
        if (GetBattlerSide(battler) != B_SIDE_PLAYER)
        {
            ...
            if (gBattleSpritesDataPtr->battlerData[battler].transformSpecies == SPECIES_BULBASAUR)
                gSprites[gBattlerSpriteIds[battler]].anims = gMonFrontAnimsPtrTable[SPECIES_BULBASAUR];
        }
